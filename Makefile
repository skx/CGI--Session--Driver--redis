# This Makefile is for the CGI::Session::Driver::redis extension to perl.
#
...
#     ABSTRACT => q[Persistent session data in CGI applications]
#     AUTHOR => [q[Steve Kemp <steve@steve.org.uk>]]
#     BUILD_REQUIRES => {  }
#     NAME => q[CGI::Session::Driver::redis]
#     PREREQ_PM => { Test::More=>undef, CGI::Session=>q[4], Redis=>undef }
#     VERSION_FROM => q[lib/CGI/Session/Driver/redis.pm]
...
NAME_SYM = CGI_Session_Driver_redis
VERSION = 0.3
...
MAKE = make
FULLEXT = CGI/Session/Driver/redis
BASEEXT = redis
PARENT_NAME = CGI::Session::Driver
DLBASE = $(BASEEXT)
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/CGI/Session/Driver/redis.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/CGI/Session/Driver
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/CGI/Session/Driver
...
TO_INST_PM = lib/CGI/Session/Driver/redis.pm

PM_TO_BLIB = lib/CGI/Session/Driver/redis.pm \
	blib/lib/CGI/Session/Driver/redis.pm
...
DISTNAME = CGI-Session-Driver-redis
DISTVNAME = CGI-Session-Driver-redis-0.3
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=$(MAN3EXT) --perm_rw=$(PERM_RW) \
	  lib/CGI/Session/Driver/redis.pm $(INST_MAN3DIR)/CGI::Session::Driver::redis.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:               CGI-Session-Driver-redis' >> META_new.yml
	$(NOECHO) $(ECHO) 'version:            0.3' >> META_new.yml
	$(NOECHO) $(ECHO) 'abstract:           Persistent session data in CGI applications' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '    CGI::Session:  4' >> META_new.yml
	$(NOECHO) $(ECHO) '    Redis:         ~' >> META_new.yml
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>Persistent session data in CGI applications</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Steve Kemp &lt;steve@steve.org.uk&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="CGI::Session" VERSION="4" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Redis::" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/CGI/Session/Driver/redis.pm blib/lib/CGI/Session/Driver/redis.pm 
